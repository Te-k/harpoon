    ...
    def run(self, args, plugins):
        ...
        if 'subcommand' in args:
            if args.subcommand == "search":
                        ...
                        for item in res['items']:
                            if item['sample_sha256'] not in already:
                                print("%s - %s - %s" % (
                                    item['ts'],
                                    item['sample_sha256'],
                                    "https://panacea.threatgrid.com/mask/samples/" + item['sample']
                                ))
                ...
                if len(res['items']) > 0:
                    item = res['items'][0]
                    print("Sample submitted the %s: https://panacea.threatgrid.com/mask/samples/%s" % (
                        item['submitted_at'],
                        item['id']))
                    idd = item['id']
                ...
                for d in data:
                    ...
                    if len(res['items']) > 0:
                        ...
                        for item in res['items']:
                            if item['sample_sha256'] not in already:
                                print("-%s: https://panacea.threatgrid.com/mask/samples/%s %s" % (
                                    item['ts'][:10],
                                    item['sample'],
                                    item['sample_sha256']))
                                already.append(item['sample_sha256'])
