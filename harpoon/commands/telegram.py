    ...
    * List users in a group : harpoon telegram users CHANNEL
    ...
    def add_arguments(self, parser):
        ...
        parser_a = subparsers.add_parser(
            "id", help="Query information on a phone number or handle"
        )
        parser_a.add_argument("ID", help="Id to be requested")
        parser_a.set_defaults(subcommand="id")
        parser_b = subparsers.add_parser(
            "messages", help="Get messages from a public channel"
        )
    ...
    def run(self, args, plugins):
        ...
        if not client.is_user_authorized():
            ...
            while not code_ok:
                ...
                try:
                    code_ok = client.sign_in(
                        self._config_data["Telegram"]["phone"], code
                    )
        ...
        if hasattr(args, "subcommand"):
            if args.subcommand == "id":
                try:
                    ...
                    print(
                        json.dumps(
                            res.to_dict(), sort_keys=True, indent=4, default=json_serial
                        )
                    )
                except ValueError:
                ...
                if args.format == "text":
                                ...
                                if isinstance(
                                    ...
                                    print(
                                        ...
                                        % (
                                            msg.date.isoformat(),
                                            ", ".join(
                                                [str(a) for a in msg.action.users]
                                            ),
                                        )
                                    )
                                elif isinstance(
                                    ...
                                    print(
                                        ...
                                        % (msg.date.isoformat(), msg.action.user_id)
                                    )
                                else:
                                    print(
                                        "[%s] Message Service: %s"
                                ...
                                if msg.media is None:
                                    if entity.megagroup:
                                        if msg.from_id not in users:
                                            users[msg.from_id] = client.get_entity(
                                                msg.from_id
                                            )
                                        print(
                                            "[%s][%i - @%s] %s"
                    ...
                    print(
                        json.dumps(msg, sort_keys=True, indent=4, default=json_serial)
                    )
                    if args.dump:
                        for msg in messages:
                            if msg.media is None:
                                if not os.path.exists(
                                    os.path.join(args.dump, str(msg.id) + ".jpg")
                                ):
                                    client.download_media(
                                        msg.media, os.path.join(args.dump, str(msg.id))
                                    )
                                    time.sleep(7)
                    ...
                    if entity.megagroup:
                        ...
                        w.writerow(
                            ["Date", "id", "Username", "userid", "Type", "Message"]
                        )
                        for m in messages:
                        ...
                        w = csv.writer(sys.stdout, delimiter=";")
                        w.writerow(
                            ["Date", "id", "Type", "Information", "Media", "Views"]
                        )
                        for m in messages:
                            if isinstance(m, telethon.tl.types.MessageService):
                                if isinstance(
                                ...
                                if m.media is None:
                                    ...
                                    if args.dump:
                                        if not os.path.exists(
                                            os.path.join(args.dump, str(m.id) + ".jpg")
                                        ):
                                            client.download_media(
                                                m.media,
                                                os.path.join(args.dump, str(m.id)),
                                            )
                ...
                try:
                    ...
                    if args.format == "text":
                        ...
                        print(
                            json.dumps(
                                users, sort_keys=True, indent=4, default=json_serial
                            )
                        )
                    else:
                        print("Not implemented yet, sorry!")
