...
class CommandTwitter(Command):
    ...
    **Dumps information from Twitter**
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--tweet", "-T", help="Download tweet with the given id")
        parser.add_argument(
            "--save", "-s", help="save all infos about an user and their tweets"
        )
        parser.add_argument(
            "--file",
            "-f",
    ...
    def run(self, args, plugins):
        bird = Bird(self._config_data["Twitter"])
        ...
        if args.user:
            ...
            f.close()
            print(
                "Handle;Status;Name;Id;Description;url;Location;Time zone;"
                + "UTC offset;Created at;Last Tweet;lang;Tweet count;"
                + "Favourite count;Followers count;Following count;List count;"
                + "Verified;Geo enabled;Default profile;Default profile image;Contributors Enabled"
            )
            for d in data:
                try:
                    user = bird.get_profile_information(d.strip())
                    print(
                        "%s;Active;%s;%i;%s;%s;%s;%s;%i;%s;%s;%s;%i;%i;%i;%i;%i;%s;%s;%s;%s;%s"
                        % (
                            user.screen_name,
                            user.name,
                            user.id,
                            ...
                            user.status.created_at.strftime("%m/%d/%Y %H:%M:%S")
